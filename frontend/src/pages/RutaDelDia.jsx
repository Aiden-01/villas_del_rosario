...
import {
  ...
  AlertTriangle, StickyNote, CalendarDays,
...
export default function RutaDelDia() {
  ...
  // ── Modal seguimiento ────────────────────────────────────────────────────────
  const [modalItem, setModalItem] = useState(null);       // item al que se le abre el modal
  const [modalTipo, setModalTipo] = useState(null);       // 'no_pago' | 'pago_parcial'
  ...
  // ── Guardar seguimiento (no_pago o pago_parcial) ─────────────────────────────
  const guardarSeguimiento = async () => {
    if (!fechaSeguimiento) {
      showToast("Selecciona una fecha de seguimiento", "error");
      return;
    }
    if (modalTipo === "pago_parcial") {
      const monto = Number(montoParcial);
      if (!monto || monto <= 0) {
        showToast("Ingresa el monto pagado", "error");
        return;
      }
      if (monto >= modalItem.montoCuota) {
        showToast("El monto parcial debe ser menor a la cuota completa", "error");
        return;
      }
    }
    ...
    try {
      // Si es pago parcial, registrar primero el pago parcial en pagos
      if (modalTipo === "pago_parcial") {
      ...
      if (!resSeg.ok) {
        showToast(dataSeg.message || "Error al guardar seguimiento", "error");
        return;
      }

      const msg = modalTipo === "no_pago"
        ? `No pago registrado para ${modalItem.cliente.nombres}`
        : `Pago parcial de Q${montoParcial} registrado para ${modalItem.cliente.nombres}`;
      showToast(msg, "success");
      cerrarModal();
      cargarRuta(); // recargar para reflejar cambios
    } catch {
  ...
  return (
    <div className="pt-16 text-[var(--text)]">
                        ...
                        return (
                          <div key={item.prestamoId}
                            ...
                            style={{
                              ...
                              border: item.esSeguimiento
                                ? "2px solid #d97706"   // naranja si viene de seguimiento
                                : "2px solid var(--primary)",
                            }}>

                            {/* Badge si viene de seguimiento */}
                            {item.esSeguimiento && (
                              <div className="flex items-center gap-1 text-xs font-semibold text-amber-600 mb-2 bg-amber-50 dark:bg-amber-900/20 px-2 py-1 rounded-lg w-fit">
                                <AlertTriangle size={11} />
                                Seguimiento pendiente
                                {item.seguimientoTipo === "no_pago" && " — No pagó"}
                                {item.seguimientoTipo === "pago_parcial" && " — Pago parcial"}
                              </div>
                            )}

                            {/* Nota del seguimiento si existe */}
                            ...
                            <div className="flex items-start gap-3">
                              ...
                              <div className="flex flex-col items-center gap-1 shrink-0">
                                {editandoOrden ? (
                                  <>
                                    <button onClick={() => moverCliente(globalIndex, "up")}
                                    ...
                                    <button onClick={() => moverCliente(globalIndex, "down")}
      ...
      {modalItem && (
        ...
        >
          <div
            className="w-full sm:max-w-sm mx-0 sm:mx-4 rounded-t-3xl sm:rounded-3xl shadow-2xl p-6"
            style={{
              backgroundColor: "var(--card)",
              border: "1px solid var(--card-border)",
          ...
          >
            ...
            <div className="space-y-4">
              {/* Monto parcial (solo si es pago_parcial) */}
              {modalTipo === "pago_parcial" && (
                <div>
                  <label className="text-sm font-semibold block mb-1" style={{ color: "var(--text)" }}>
                    Monto recibido
                    <span className="font-normal opacity-50 ml-1">(cuota completa: Q{Number(modalItem.montoCuota).toFixed(2)})</span>
                  </label>
                  <input
                    type="number"
              ...
              <div>
                ...
                <textarea
                  placeholder={modalTipo === "no_pago"
                    ? "Ej: No se encontraba en casa, acordó pagar el jueves..."
                    : "Ej: Solo tenía Q150, dijo que el resto lo da el miércoles..."
                  }
