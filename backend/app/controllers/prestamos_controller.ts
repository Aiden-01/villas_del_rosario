...
export default class PrestamosController {
  ...
  private async resolverLotesPrediosConLoteId(
    ...
    if (loteDuplicado) {
      throw new LoteVentaError(
        'No se puede agregar el mismo lote mas de una vez',
        400,
        loteDuplicado
      )
    }
  ...
  async update({ request, params, response }: HttpContext) {
    try {
      ...
      // Regla de integridad financiera: si la venta ya tiene movimientos financieros registrados,
      // se bloquea estrictamente la modificación de condiciones financieras (monto, cuotas)
      // tanto para trabajador como para administrador.
      const tienePagos = await Pago.query().where('venta_id', params.id).first()

      const intentaCambiarFinanciero =
        (data.monto !== undefined && Number(data.monto) !== Number(prestamo.monto)) ||
        (data.cuotas !== undefined && Number(data.cuotas) !== Number(prestamo.cuotas))

      if (tienePagos && intentaCambiarFinanciero) {
        return response.forbidden({
          message:
            'No se pueden modificar las condiciones financieras (monto o cuotas) de una venta con movimientos financieros registrados. Requiere un proceso de reestructuracion financiera.',
        })
      }
  ...
  async destroy({ request, params, response }: HttpContext) {
    try {
      ...
      for (const loteId of lotesIds) {
        // Liberar lote solo si no tiene otra venta activa
