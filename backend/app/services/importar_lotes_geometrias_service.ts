...
type Posicion2D = [number, number]
type CoordenadasPoligono = Posicion2D[][]

type LoteGeoJsonNormalizado = {
  codigo: string
  loteNumero: string
  areaFuente: number
  geometria: {
...
type FilaPreparada = LoteGeoJsonNormalizado & {
  loteId: number
  areaSistema: number
  diferenciaArea: number
}

export type ResultadoImportacionLote = {
  lote: string
  codigo: string
  areaSistema: number
  areaFuente: number
  diferenciaArea: number
  resultado: 'LISTO' | 'IMPORTADO'
}

export type ResultadoImportacionLotes = {
  dryRun: boolean
  total: number
  filas: ResultadoImportacionLote[]
}
...
function textoObligatorio(valor: unknown, campo: string, indice: number) {
...
function numeroLoteObligatorio(valor: unknown, indice: number) {
...
function normalizarPoligono(coordenadas: unknown, indice: number): CoordenadasPoligono {
  if (!Array.isArray(coordenadas)) {
    throw new Error(`Feature ${indice + 1}: coordenadas MultiPolygon invalidas`)
  }
  if (coordenadas.length !== 1) {
    throw new Error(
      `Feature ${indice + 1}: MultiPolygon debe contener exactamente una parte; contiene ${coordenadas.length}`
    )
  }
  ...
  return poligono.map((anillo, indiceAnillo) => {
    ...
    const anillo2D = anillo.map((posicion, indicePosicion): Posicion2D => {
      if (
        ...
        throw new Error(
          `Feature ${indice + 1}: posicion ${indicePosicion + 1} del anillo ${indiceAnillo + 1} invalida`
        )
      }
...
export function validarYNormalizarGeoJsonLotes(documento: unknown): LoteGeoJsonNormalizado[] {
  ...
  if (!Array.isArray(documento.features) || documento.features.length !== TOTAL_FEATURES_LOTES) {
    ...
    throw new Error(
      `Se requieren exactamente ${TOTAL_FEATURES_LOTES} features; se recibieron ${total}`
    )
  }
...
export function parsearAreaSistema(valor: unknown, loteNumero: string) {
  ...
  if (!coincidencia) {
    throw new Error(
      `Lote ${loteNumero}: area del sistema no tiene un formato interpretable: ${valor}`
    )
  }
...
async function validarGeometriaPostgis(trx: TransactionClientContract, fila: FilaPreparada) {
  ...
  const resultado = validacion.rows[0] as
    | {
        tipo: string
        srid: number
        dimensiones: number
  ...
  if (
    ...
    throw new Error(
      `Lote ${fila.loteNumero}: geometria convertida invalida (${resultado?.motivo || 'sin detalle'})`
    )
  }
}

async function prepararFilas(
  trx: TransactionClientContract,
  features: LoteGeoJsonNormalizado[],
  dryRun: boolean
): Promise<FilaPreparada[]> {
  ...
  if (!dryRun) consultaLotes.forUpdate()
  ...
  const filas = features.map((feature): FilaPreparada => {
    ...
    if (diferenciaArea > TOLERANCIA_AREA_M2 + 1e-9) {
      throw new Error(
        `Lote ${feature.loteNumero}: area incompatible; sistema=${areaSistema.toFixed(2)} m2, fuente=${feature.areaFuente.toFixed(2)} m2, diferencia=${diferenciaArea.toFixed(2)} m2`
      )
    }
...
export async function importarGeometriasLotes(
  documento: unknown,
  opciones: { dryRun?: boolean } = {}
): Promise<ResultadoImportacionLotes> {
...
export async function importarGeometriasLotesDesdeArchivo(
  rutaArchivo: string,
  opciones: { dryRun?: boolean } = {}
) {
