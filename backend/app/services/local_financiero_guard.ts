...
export function validarPermisoVerificacion(entorno: EntornoFinanciero, confirmProduction = false) {
  if (entorno.NODE_ENV === 'production') {
    if (!confirmProduction) {
      throw new Error(
        'Verificacion financiera en produccion bloqueada: vuelve a ejecutar con --confirm-production'
      )
    }
  ...
  if (
    ...
    throw new Error(
      'Verificacion financiera bloqueada: fuera de production solo se permite en las bases locales dev/test'
    )
  }
