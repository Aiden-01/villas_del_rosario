...
function fechaProgramadaVenta(venta, numeroCuota) {
    const base = venta.fechaCobro || venta.fechaInicio;
    if (!base)
        return null;
    const baseDate = typeof base.plus === 'function'
        ? DateTime.fromObject({
            year: base.year,
            month: base.month,
            day: base.day,
...
function construirPendiente(venta) {
...
async function obtenerCobrosDeLaFecha(fecha) {
    ...
    return ventas
        ...
        .filter((item) => Boolean(item && item.fechaProgramada === fecha));
}
...
function construirCorreo(fecha, cobros) {
...
export async function notificarCobrosDelDia(fechaInput) {
...
//# sourceMappingURL=notificaciones_cobros_service.js.map