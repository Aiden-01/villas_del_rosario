...
export function verificarEntornoFinancieroLocal(testOnly = false) {
    ...
    if (!['development', 'test'].includes(nodeEnv) ||
        ...
        port === 5432 ||
        !(testOnly ? ['villas_del_rosario_test'] : ['villas_del_rosario_dev', 'villas_del_rosario_test']).includes(database) ||
        (testOnly && nodeEnv !== 'test')) {
        throw new Error('Operacion financiera local bloqueada: verificar NODE_ENV, DB_HOST, DB_PORT (no usar 5432) y DB_DATABASE');
    }
...
export function validarPermisoBackfill(entorno, confirmProduction = false) {
    if (entorno.NODE_ENV === 'production') {
        if (!confirmProduction) {
            throw new Error('Backfill en produccion bloqueado: vuelve a ejecutar con --confirm-production');
        }
        return entorno;
    }
    ...
    if (!['development', 'test'].includes(entorno.NODE_ENV) ||
        ...
        !basePermitida) {
        throw new Error('Backfill bloqueado: fuera de production solo se permite en las bases locales dev/test');
    }
    return entorno;
}
//# sourceMappingURL=local_financiero_guard.js.map