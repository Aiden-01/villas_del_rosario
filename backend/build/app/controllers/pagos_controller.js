...
export default class PagosController {
    ...
    fechaIso(fecha) {
        try {
            if (!fecha)
                return null;
            if (typeof fecha.toISODate === 'function')
    ...
    fechaProgramadaVenta(venta, numeroCuota) {
        const base = venta.fechaCobro || venta.fechaInicio;
        if (!base)
            return null;
        let dt;
        if (typeof base.plus === 'function') {
            const baseDate = base;
    ...
    voucherPago(params) {
        ...
        return {
            ...
            venta: {
                ...
                pendienteCuotaActual: params.pendienteCuotaRestante === undefined
    ...
    async resolverProgramaciones(ventaId, numeroCuota) {
    ...
    async agenda({ request, response }) {
        try {
            ...
            const pagosDelDia = await Pago.query()
                ...
                .preload('predios', (predios) => predios.preload('lote'))
                .preload('pagos'));
            const gestionadosKeys = new Set(programacionesDelDia.map((programacion) => `${programacion.prestamoId}-${programacion.numeroCuota}`));
            const pendientes = [];
            for (const venta of ventas) {
                const pendiente = this.construirPendiente(venta);
                if (!pendiente)
                    continue;
                if (pendiente.fechaProgramada !== fecha)
                    continue;
                if (gestionadosKeys.has(`${venta.id}-${pendiente.proximaCuota}`))
                    continue;
                pendientes.push(pendiente);
            }
            const cobradosAgrupados = new Map();
            for (const pago of pagosDelDia) {
                if (gestionadosKeys.has(`${pago.prestamoId}-${pago.numeroCuota}`))
                    continue;
            ...
            return response.ok({
                ...
                totalRecaudado: Number(cobrados.reduce((sum, current) => sum + Number(current.totalPagadoHoy), 0).toFixed(2)),
                pendientes,
                cobrados,
                gestionados,
            });
    ...
    construirPendienteConPagosActivos(venta, programaciones) {
        const resumen = this.resumenCuotas(venta);
        if (!resumen.proximaCuota)
            return null;
    ...
    async byPrestamo({ request, params, response }) {
        try {
            const authHeader = request.header('authorization');
            const user = await this.verifyToken(authHeader || '');
            if (!user)
        ...
        try {
            ...
            return response.created({
                ...
                voucher: this.voucherPago({
                    ...
                    aplicaciones: resultado.aplicaciones,
                    pendienteCuotaRestante: Number(Math.max(resumenAntes.montoPendienteCuota - monto, 0).toFixed(2)),
    ...
    async programar({ request, response }) {
        try {
            const authHeader = request.header('authorization');
            const user = await this.verifyToken(authHeader || '');
            if (!user)
                return response.forbidden({ message: 'No autorizado' });
            const data = await programacionPagoValidator.validate(cleanEmptyStrings(request.all(), ['montoPagado']));
            const ventaId = data.ventaId || data.prestamoId;
            ...
            await registrarActividad({
                ...
                descripcion: data.tipoGestion === 'no_pago'
            ...
            return response.created({
                ...
                programacion,
                voucher: data.tipoGestion === 'pago_parcial' && pagoParcial
                    ? this.voucherPago({
                        venta: prestamo,
                        ...
                        numeroCuota: resumen.proximaCuota,
                        pendienteCuotaRestante: Number(Math.max(resumen.montoPendienteCuota - montoParcial, 0).toFixed(2)),
    ...
    async anular({ request, params, response }) {
        try {
            const authHeader = request.header('authorization');
            const user = await this.verifyToken(authHeader || '');
            if (!user)
            ...
            const venta = pago.prestamo;
            const { reconstruirAplicacionesVenta } = await import('#services/reconstruir_aplicaciones_service');
...
//# sourceMappingURL=pagos_controller.js.map