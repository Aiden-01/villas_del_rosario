...
export default class NotificarCobrosCommand extends BaseCommand {
    ...
    async run() {
        ...
        if (resultado.dryRun) {
            this.logger.info(`[dry-run] ${resultado.encontrados} cobros encontrados para ${resultado.fecha}. No se envio correo ni se guardo historial.`);
            completed = true;
            return;
        }
        if (resultado.enviados === 0) {
            this.logger.info(`No hay cobros nuevos por notificar para ${resultado.fecha}. Omitidos: ${resultado.omitidos}`);
            completed = true;
            return;
        }
        this.logger.success(`Correo enviado con ${resultado.enviados} cobros para ${resultado.fecha}. Omitidos por historial: ${resultado.omitidos}`);
        completed = true;
    }
}
//# sourceMappingURL=notificar_cobros.js.map